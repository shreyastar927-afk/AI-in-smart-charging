...
|----------|-------------|
| `P[i, t]` | Power (kW) allocated to vehicle `i` at time slot `t` |
| `D[i, t]` | Power (kW) donated back to the pool by V2G-eligible vehicle `i` at slot `t` |
| `slack[i]` | Soft constraint absorber — allows minimum viable charge to be missed at a high penalty cost rather than making the problem infeasible |
| `donor_slack[i]` | Safety absorber for V2G donors — prevents infeasibility when a donor's own received charge is uncertain |
...
```python
W[i] = (tier * 20) + (demand / hours_left) + (fairness_debt * 5) + token_boost
```
...
```python
token_boost[i] = tokens_spent[i] * TOKEN_TO_POINTS_RATE
```
